...
sensor.set_brightness(8500)  # 设置相机图像亮度。-3至+3。
sensor.skip_frames(time=20)  # 让相机图像在改变相机设置后稳定下来
...
animal_fruit_net_path = "_qm_model_04_1.0000_2000.nncu"
number_labels = [line.rstrip() for line in open("/sd/labels_number.txt")]  # 加载标签
animal_fruit_labels = [line.rstrip() for line in open("/sd/labels_animal_fruits.txt")]
...
def continuity_verify(num):
    ...
    if verify_i >= 3 and num in [1, 2]:  # 云台左转或者右转
        ...
        while True:
            ...
            for r in img.find_rects(roi=(0, 30, 160, 70), threshold=8000):  # 在图像中搜索矩形
                w = r.w()
                h = r.h()
                ...
                img2 = img.copy(r.rect())  # 拷贝矩形框内的图像
                        ...
                        if count_fruit >= 3: #  水果计数三次
                            ...
                            while True:
                                ...
                                if uart_back_num:
                                    ...
                                    if uart_back == b'\x0C' or uart_back == b'\x0D':
                                        break  # 1为云台转向完成，2为舵机转向完成, 3为等待完成， 4为打靶完成
                                for r in img_1.find_rects(roi=(0, 30, 160, 70), threshold=8000):  # 在图像中搜索矩形
        ...
        number_flag = False
        if check_out:
            print('number is over')
        uart_array[2] = 0  # number置零
    else:
        img = sensor.snapshot()
        uart.write(bytearray(uart_array))  # 发送无意义的0000刷新下位机

    # print("连续性验证结果为：%d" % uart_array[0])


"""apriltag识别"""


def apriltag_recognition():
    global apriltag_flag  # 是否进入apriltag识别
    global number_flag  # 是否进入数字识别

    tag_all = img.find_apriltags(families=image.TAG25H9)
    if tag_all:
        for tag in tag_all:  # 指定apriltag类型为TAG25H9
            img.draw_rectangle(tag.rect(), color = (255, 0, 0))     # 找到apriltag后绘制矩形框，将apriltag框起来
            print("tag结果：%d" %tag.id())
            if tag.id() % 2 != 0:
                continuity_verify(2)  # apriltag奇数舵机右转（2）
                #red.on()  # 成功运行，打开红灯
            else:
                continuity_verify(1)  # apriltag偶数舵机左转（1）
                #red.on()  # 成功运行，打开红灯
...
# def animal_fruit_recognition():
#     global apriltag_flag  # 是否进入apriltag识别
#     global animal_fruit_flag  # 是否进入动物水果识别
...
#         img.draw_cross(m, n, color=(255, 0, 0))
#
#         # 将矩形框内的图像使用训练好的模型进行分类
#         # net.classify()将在图像的roi上运行网络(如果没有指定roi，则在整个图像上运行)
#         # 将为每个位置生成一个分类得分输出向量。
...
#
#         # 默认设置只是进行一次检测...更改它们以搜索图像...
#         for obj in nncu.classify(animal_fruit_net, img2, min_scale=1.0, scale_mul=0.5, x_overlap=0.0, y_overlap=0.0):
#             # print("**********\nTop 1 Detections at [x=%d,y=%d,w=%d,h=%d]" % obj.rect())
#             sorted_list = sorted(zip(animal_fruit_labels, obj.output()), key=lambda x: x[1], reverse=True)
#             for i in range(1):
#                 print("%s = %f" % (sorted_list[i][0], sorted_list[i][1]))
...
while True:
    ...
    img = sensor.snapshot()
    number_recognition()
    # apriltag_recognition()
    # uart_back_num = uart.any()  # 获取当前串口数据数量
    # if uart_back_num:
    #     uart_back_main = uart.read(uart_back_num)  # 读取串口数据
    #     if check_out:
    #         print(uart_back_main)
    #     if uart_back_main == b'\xB0':  # A0为apriltag,B0为number
    #         number_flag = True
    #         while number_flag:
    #             number_recognition()
    # else:
