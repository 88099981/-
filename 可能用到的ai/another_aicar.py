...
sensor.set_brightness(1500)  # 设置相机图像亮度。-3至+3。
...
def number_recognition():
    ...
    #temp.to_grayscale()
    ## print(temp.format())
    #for r in temp.find_rects(threshold=30000):  # 在图像中搜索矩形
        #w = r.w()
        #h = r.h()
        #w1 = abs((w - h) / ((w + h) / 2))
        #rect_area = w * h
...
while True:
    ...
    if uart_back_num:
        ...
        if uart_back_main == b'\xA0':  # A0为apriltag,B0为number
            apriltag_flag = True
            while apriltag_flag:
                apriltag_recognition()
        elif uart_back_main == b'\xB0':  # A0为apriltag,B0为number
            number_flag = True
            while number_flag:
                number_recognition()
    else:
