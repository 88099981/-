...
uint8 Connect(EDGE Target[],uint8 l_or_r,uint8 p1_y,uint8 p2_y) //通过因为高度和边沿点是对应的，所以输入纵坐标来表示对应的点 //ATTENTION p1_y小于p2_y
{
    float Slope=0;   //斜率 其实是cot
    if(l_or_r)
    {
...
uint8 Mid_Connect(int16 Target[],uint8 p1_y,uint8 p2_y) //通过因为高度和边沿点是对应的，所以输入纵坐标来表示对应的点    注意p1_y小于p2_y
{                                                      //重载 因为mid为int16类型
    float Slope=0;   //斜率 其实是cot
